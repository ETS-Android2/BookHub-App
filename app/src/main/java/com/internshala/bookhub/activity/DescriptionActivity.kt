...
class DescriptionActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.title = "Book Details"  // Till here..
        ...
        jsonParams.put("book_id", bookId) // Send book_id for id POST request

        if(ConnectionManager().checkConnectivity(this@DescriptionActivity)) {   // CHECK FOR INTERNET CONNECTIVITY !!

            val jsonRequest = object : JsonObjectRequest(Request.Method.POST, url, jsonParams, Response.Listener {

                //  Here we handle the response using - "TRY & CATCH"
                try {
                    val success = it.getBoolean("success") // Declare a variable to store the value of a key - success

                    if (success) {
                        val bookJsonObject = it.getJSONObject("book_data")
                        progressLayout.visibility = View.GONE

                        val bookImageUrl = bookJsonObject.getString("image")// for storing book Image, we declare a another variable here
                        ...
                        txtBookDesc.text = bookJsonObject.getString("description")
                                          ...
                                          // "Set Add to Fav or Remove to Fav button" !!
                        ...
                        if(isFav){
                            ...
                            btnAddToFav.setBackgroundColor(nofavColor)
                               ...
                               // "Set onClickListener() method on - Add to Fav or Remove to Fav button"
            ...
            dialog.setNegativeButton("Exit"){text, listener ->
                ActivityCompat.finishAffinity(this@DescriptionActivity) // This line will close the app completely.
            }
            dialog.create()
            dialog.show()
        }
                            ...
                            // WORKING OF ASYNC TASK (For Adding Book on Favourites or remove from Favourites) !!

    class DBAsyncTask(val context: Context, val bookEntity: BookEntity, val mode: Int) : AsyncTask<Void, Void, Boolean>(){
        /*
        Mode 1 -> Check DB if thr book is favourite or not
        Mode 2 -> Save the book into DB as favourite
        Mode 3 -> Remove the favourite book
         */

        val db = Room.databaseBuilder(context,BookDatabase::class.java,"books-db").build()  // Initialise the db class(initialise it globally
        ...
        override fun doInBackground(vararg p0: Void?): Boolean {
            ...
            when(mode) {
                ...
                1 ->{
                    ...
                    // Check DB if thr book is favourite or not
